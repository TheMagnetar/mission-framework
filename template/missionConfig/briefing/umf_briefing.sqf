...
switch (_unitFaction) do {
    ...
    case "blu_f": {
        #include "umf_briefing_blu_f.sqf"
    };

    // Briefing for CTRG faction (APEX).
    case "blu_ctrg_f": {
        #include "umf_briefing_blu_ctrg_f.sqf"
    };

    // Briefing for GENDARMERIE faction (APEX).
    case "blu_gen_f": {
        #include "umf_briefing_blu_gen_f.sqf"
    };

    // Briefing for BLUFOR Pacific faction (APEX).
    case "blu_t_f": {
        #include "umf_briefing_blu_t_f.sqf"
    };

    // Briefing for FIA faction.
    case "blu_g_f": {
        #include "umf_briefing_fia_f.sqf"
    };
    ...
    case "opf_f": {
        #include "umf_briefing_opf_f.sqf"
    };

    // Briefing for OPFOR Pacific faction (APEX).
    case "opf_t_f": {
        #include "umf_briefing_opf_t_f.sqf"
    };

    // Briefing for Independent faction.
    case "ind_f": {
        #include "umf_briefing_ind_f.sqf"
    };

    // Briefing for Syndikat faction (APEX).
    case "ind_c_f": {
        #include "umf_briefing_ind_c_f.sqf"
    };

    // Briefing for Civilian faction.
    case "civ_f": {
        #include "umf_briefing_civ_f.sqf"
    };
    ...
    case "bwa3_faction": {
        #include "umf_briefing_bwa3_faction.sqf"
    };
    ...
    case "fow_usmc": {
        #include "umf_briefing_fow_usmc.sqf"
    };

    case "logic": {
        #include "umf_briefing_logic.sqf"
    };

    default {
        _recognised = false;
        #include "umf_briefing_fallback_faction.sqf"
    };
};

if (!_recognised) then {
    player globalchat format ["ERROR (umf_briefing.sqf): Faction %1 is not defined. Using fallback faction.", _unitFaction];
};
